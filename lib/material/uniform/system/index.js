...
function applyWorldTransformUniform(renderArgs, uniformDesc, drawRequest) {
    // TODO: Create world transform for object
    renderArgs.gl.uniformMatrix4fv(uniformDesc.location, false, WORK_MATRIX);
}

function applyWorldViewTransformUniform(renderArgs, uniformDesc, drawRequest) {
    // TODO: Create world-view transform for object
    renderArgs.gl.uniformMatrix4fv(uniformDesc.location, false, WORK_MATRIX);
}

function applyWorldViewProjectionTransformUniform(renderArgs, uniformDesc, drawRequest) {
    renderArgs.gl.uniformMatrix4fv(uniformDesc.location, false, WORK_MATRIX);
}

function applyWorldPositionUniform(renderArgs, uniformDesc, drawRequest) {
    renderArgs.gl.uniform3fv(uniformDesc.location, drawRequest.worldPosition);
}
