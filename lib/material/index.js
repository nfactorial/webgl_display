...
export default class Material {
    ...
    initialize(gl) {
        ...
        for (let loop = 0; loop < count; ++loop) {
            ...
            const uniform = createUniform(gl, this.program, desc.name);
            if (uniform.shared) {
    ...
    createInstance() {
        ...
        this.instanceUniforms.forEach(uniform => instance.uniforms.push(uniform.clone()));
    ...
    onBeginRender(renderArgs) {
        ...
        this.sharedUniforms.forEach(uniform => uniform.apply(renderArgs));
    }
