...
export default class DrawRequestQueue {
    ...
    execute(renderArgs) {
        if (this.count) {
            const uniformCount = this.materialInstance.instanceUniforms.length;
            ...
            for (let loop = 0; loop < count; loop++) {
                ...
                for (let uniform = 0; uniform < uniformCount; uniform++) {
                    this.materialInstance.instanceUniforms[uniform].apply(
                        renderArgs,
                        // TODO: Need to check, isn't the 'this' bound to the function when invoking like this?
                        this.materialInstance.instanceUniforms[uniform],
                        request
                    );
                }
