...
export default class Material {
    constructor() {
        ...
        this.phase = '';
        this.program = null;
        this.depthComparison = DepthCompare.Less;
    ...
    initialize(gl) {
        ...
        for (let loop = 0; loop < count; ++loop) {
            ...
            const uniform = createUniform(gl, this.program, desc.name, this.systemUniforms);
            if (uniform.shared) {
    ...
    createInstance() {
        ...
        for (let loop = 0, len = this.instanceUniforms.length; loop < len; loop++) {
    ...
    onBeginRender(renderArgs) {
        ...
        for (let loop = 0; loop < count; loop++) {
            // TODO: Need to check, isn't the 'this' bound to the function when invoking like this?
            this.sharedUniforms[loop].apply(renderArgs, this.sharedUniforms[loop]);
        }
    }
