        ...
        if (count) {
            for (let loop = 0; loop < count; loop += 1) {
                ...
                if (request.vertexArray) {
                    renderArgs.state.bindVertexArray(request.vertexArray.id);
                } else {
                    renderArgs.state.bindVertexArray(null);
                    renderArgs.state.bindArrayBuffer(request.vertexBuffer.id);
                    // NOTE: This wont work if the vertex buffer has changed, but the attribute buffer hasn't
                    renderArgs.state.enableAttributes(this.material.program.attributeBuffer);
                }
